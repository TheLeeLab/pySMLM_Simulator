...
class IO_Functions:
    ...
    def save_simulation_params(self, analysis_p_directory, to_save):
        ...
        self.save_as_json(
            to_save, os.path.join(analysis_p_directory, "simulation_params.json")
        )
        return
    ...
    def write_gif(self, volume, file_path, fps=100, loop=0, two_images=True):
        ...
        if two_images == True:
            ...
            volume[:mid_point, :, :] = (
                (volume[:mid_point, :, :] - minhalfone)
                / (maxhalfone - minhalfone)
                * 255.9
            ).astype(np.uint8)
            ...
            volume[mid_point:, :, :] = (
                (volume[mid_point:, :, :] - minhalftwo)
                / (maxhalftwo - minhalftwo)
                * 255.9
            ).astype(np.uint8)

        else:
            volume = (
                (volume - np.min(volume)) / (np.max(volume) - np.min(volume)) * 255.9
            ).astype(np.uint8)

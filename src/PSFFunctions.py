...
class PSF_Functions:
    ...
    def gaussian2d_PSF(
        self, x, y, sigma_psf, x0, y0, n_photons=4000, bitdepth=np.float32
    ):
        """
        ...
        """
        X, Y = np.meshgrid(x, y)
        nfactor = np.multiply(sigma_psf, np.sqrt(2.0 * np.pi))
        if isinstance(x0, float):
            PSF_g2d = np.asarray(
                np.multiply(
                    np.divide(np.random.poisson(n_photons), nfactor),
                    np.exp(
                        np.subtract(
                            -np.divide(
                                np.square(X - x0),
            ...
            for i in np.arange(len(x0)):
                PSF_g2d = PSF_g2d + np.asarray(
                    np.multiply(
                        np.divide(photon_numbers[i], nfactor),
                        np.exp(
                            np.subtract(
                                -np.divide(
                                    np.square(X - x0[i]),
    ...
    def generate_noisy_image_matrix(
        self, image_size, lambda_sensor, mu_sensor, sigma_sensor, bitdepth=np.float32
    ):
        """

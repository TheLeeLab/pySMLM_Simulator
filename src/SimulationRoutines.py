...
class Simulation_Routines:
    ...
    def generate_tiffs_and_gifs(
        ...
        to_save = {
            "n_photons_per_loc": n_photons,
            "n_frames": n_frames,
            "labelling_density": labelling_density,
        ...
        IO.save_simulation_params(simulation_p_directory, to_save)
        ...
        superres_image_stack, superres_cumsum_stack, dl_image, superres_image = (
            PSF_F.generate_superres_stack(
                image.shape,
                indices,
                n_photons=n_photons,
                n_frames=n_frames,
        ...
        if invert == True:  # if true, invert all of these values
            ...
            superres_image_stack = (
                (superres_image_stack - np.min(superres_image_stack))
                / (np.max(superres_image_stack) - np.min(superres_image_stack))
                * max_value
            )
            superres_image_stack = -superres_image_stack + max_value
            superres_cumsum_stack = (
                (superres_cumsum_stack - np.min(superres_cumsum_stack))
                / (np.max(superres_cumsum_stack) - np.min(superres_cumsum_stack))
                * max_value
            )
            superres_cumsum_stack = -superres_cumsum_stack + max_value
            dl_image = (
                (dl_image - np.min(dl_image))
                / (np.max(dl_image) - np.min(dl_image))
                * max_value
            )
            dl_image = -dl_image + max_value
            superres_image = (
                (superres_image - np.min(superres_image))
                / (np.max(superres_image) - np.min(superres_image))
                * max_value
            )
        ...
        gif_filepath = os.path.join(
            simulation_directory, raw_filename + "_superres.gif"
        )
        tiff_filepath = os.path.join(
            simulation_directory, raw_filename + "_superres.tiff"
        )
        tiff_sr_filepath = os.path.join(
            simulation_directory, raw_filename + "_superres_singleimage.tiff"
        )
        tiff_dl_filepath = os.path.join(
            simulation_directory, raw_filename + "_diffractionlimited.tiff"
        )
